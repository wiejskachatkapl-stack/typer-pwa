...
// BUILD 3073: system buttons consistent with the rest of the game
...
// ===== BUILD 3073: numer gracza jest główną tożsamością w pokoju =====
...
function scheduleRoomIdentityRepair(){
  // BUILD 3073: wyłączono automatyczne usuwanie duplikatów UID.
...
async function buildSeasonPodiumCanvas(ev){
  ...
  ctx.fillText(room ? `${room}  •  TYPER v.3.073` : "TYPER v.3.073",800,850);
  return canvas;
}
...
function renderMatches(){
  ...
  // BUILD 3073: licznik jest w stałym dolnym pasku poza przewijaną listą meczów.
  updateTypingDeadlineUI();
...
function ensureEndRoundConfirmModal(){
  ...
  // BUILD 3073: systemowe przyciski TAK/NIE zgodne z resztą gry.
...
(async()=>{
  try{
    setBg(BG_HOME);
    setFooter(`Mariusz Gębka v.3.073`);
