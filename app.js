...
// BUILD 3074: system buttons consistent with the rest of the game
...
// ===== BUILD 3074: numer gracza jest główną tożsamością w pokoju =====
...
function scheduleRoomIdentityRepair(){
  // BUILD 3074: wyłączono automatyczne usuwanie duplikatów UID.
...
async function buildSeasonPodiumCanvas(ev){
  ...
  ctx.fillText(room ? `${room}  •  TYPER v.3.074` : "TYPER v.3.074",800,850);
  return canvas;
}
...
function renderMatches(){
  ...
  // BUILD 3074: licznik jest w stałym dolnym pasku poza przewijaną listą meczów.
  updateTypingDeadlineUI();
...
function ensureEndRoundConfirmModal(){
  ...
  // BUILD 3074: systemowe przyciski TAK/NIE zgodne z resztą gry.
...
(async()=>{
  try{
    setBg(BG_HOME);
    setFooter(`Mariusz Gębka v.3.074`);
