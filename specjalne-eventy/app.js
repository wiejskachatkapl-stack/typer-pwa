...
function showScreen(id){
  ...
  // BUILD 3087: osobna klasa stabilizująca układ pokoju na telefonie.
...
function updateActiveEventButton(){
  ...
  if(currentRoomCode) updateRoomEventButtonState().catch(()=>{});
}
...
async function openActiveEventModule(){
  ...
  const roomEventConfig = await wcGetEventConfig();
  if(roomEventConfig.enabled === false && !isAdmin()){
    showToast(getLang()==='en' ? 'The Event is inactive.' : 'Event jest nieaktywny.');
    await updateRoomEventButtonState();
    return;
  }
  window.__wcEventConfig = roomEventConfig;
  try{
...
function getModernBtnSpec(btnName){
  ...
  const map = {
    ...
    "btn_settings.png":          {pl:"Ustawienia Eventu", en:"Event settings", icon:"ico-manual2", variant:"blue", wide:true}
  };
  return map[n] || null;
}
...
// BUILD 3087: system buttons consistent with the rest of the game
...
// ===== BUILD 3087: numer gracza jest główną tożsamością w pokoju =====
...
function scheduleRoomIdentityRepair(){
  // BUILD 3087: wyłączono automatyczne usuwanie duplikatów UID.
...
function wcEnsureEventStyles(){
  ...
  st.textContent = `
    ...
    /* v1008: na komputerze przyciski Eventu wypełniają całą szerokość od lewej do prawej */
    #modal.worldcupMode .wcTopActionBar{
      width:100%;min-width:0;display:grid;
      grid-template-columns:repeat(8,minmax(0,1fr));
      align-items:stretch;justify-content:stretch;
      gap:7px;padding:2px 0 3px 0;flex:0 0 auto;
    }
    /* Kontener admina nie zajmuje osobnej kolumny — wszystkie 8 przycisków tworzy jeden równy rząd. */
    #modal.worldcupMode .wcTopAdminButtons{display:contents !important;}
    #modal.worldcupMode .wcTopActionBar .wcBtnImg{
      width:100% !important;min-width:0 !important;max-width:none !important;
      min-height:40px !important;padding:6px 5px !important;
      font-size:clamp(9.5px,.75vw,11px) !important;
      line-height:1.03 !important;white-space:nowrap;gap:4px !important;
    ...
    /* v1012: stały dół panelu meczów oraz przestrzeń zaprojektowana pod maksymalnie 10 spotkań. */
    #modal.worldcupMode .wcEventCenter{
    ...
    #modal.worldcupMode .wcEventCenter #wcMatchesList{
      ...
      scrollbar-color:rgba(79,169,255,.72) rgba(0,0,0,.16);
    }
    ...
    #modal.worldcupMode .wcEventFooter #wcSavePicksBtn{
      transition:opacity .16s ease,visibility .16s ease,filter .16s ease !important;
    }
    @media (min-width:981px) and (min-height:621px){
      #modal.worldcupMode .wcEventGrid{
        height:clamp(500px,calc(94vh - 238px),575px) !important;
      ...
      #modal.worldcupMode .wcEventSummary .wcEventTablePanel{
        margin-top:auto !important;
      }
      #modal.worldcupMode .wcPickRow{
        min-height:30px !important;
        padding:2px 6px !important;
        gap:4px !important;
        grid-template-columns:minmax(180px,1fr) 100px 146px !important;
      }
      #modal.worldcupMode .wcPickRow .teamName{
        font-size:12.5px !important;
        line-height:1.05 !important;
      }
      #modal.worldcupMode .wcPickRow .scoreInput{
        width:32px !important;
        height:25px !important;
        font-size:12px !important;
      }
      #modal.worldcupMode .wcPickRow .resultPill{
    ...
    /* v1016: układ paneli dopasowany wizualnie do głównego Typera. */
    #modal.worldcupMode .modalCard{
      width:min(1540px,98vw) !important;
      height:min(830px,96vh) !important;
      max-height:96vh !important;
      border-radius:26px !important;
      background:linear-gradient(180deg,rgba(5,20,48,.985),rgba(3,13,34,.985)) !important;
      box-shadow:0 24px 70px rgba(0,0,0,.62),inset 0 1px 0 rgba(255,255,255,.07) !important;
    }
    ...
    #modal.worldcupMode .wcEventStatusBar{
      ...
      display:grid !important;
      grid-template-columns:minmax(300px,1.45fr) auto minmax(205px,.9fr) minmax(170px,.72fr) minmax(105px,.42fr) minmax(105px,.42fr);
      align-items:center !important;
      gap:8px !important;
      padding:9px 12px !important;
      border-radius:18px !important;
      border:1px solid rgba(91,154,229,.28) !important;
      background:linear-gradient(180deg,rgba(11,37,80,.92),rgba(5,23,55,.9)) !important;
    ...
    #modal.worldcupMode .wcTopActionBar{
      ...
      gap:7px !important;
      border-radius:18px !important;
      border:1px solid rgba(91,154,229,.25) !important;
      background:linear-gradient(180deg,rgba(10,35,76,.88),rgba(4,20,49,.9)) !important;
      box-shadow:inset 0 1px 0 rgba(255,255,255,.04),0 7px 16px rgba(0,0,0,.14) !important;
    }
    #modal.worldcupMode .wcTopActionBar .wcBtnImg{
    ...
    #modal.worldcupMode .wcEventGrid{
      grid-template-columns:minmax(310px,.9fr) minmax(560px,1.72fr) minmax(325px,.96fr) !important;
      gap:10px !important;
    ...
    #modal.worldcupMode .wcEventPlayersPanel{
      ...
      border:1px solid rgba(91,154,229,.28) !important;
      background:linear-gradient(180deg,rgba(9,31,70,.93),rgba(4,20,49,.94)) !important;
      box-shadow:inset 0 1px 0 rgba(255,255,255,.045),0 12px 26px rgba(0,0,0,.18) !important;
    }
    #modal.worldcupMode .wcEventSummary{
      display:grid !important;
    ...
    #modal.worldcupMode .wcEventSummary .wcRoundTop3Card{
      border-top:1px solid rgba(125,176,235,.18) !important;
    }
    #modal.worldcupMode .wcEventSummary .wcTop3Head{
      margin-bottom:8px !important;
      padding:0 3px 7px !important;
      border-bottom:1px solid rgba(125,176,235,.12);
    }
    ...
    #modal.worldcupMode .wcEventSummary .wcTop3Row{
      ...
      background:rgba(2,15,38,.48) !important;
      border-color:rgba(118,169,226,.16) !important;
    }
    ...
    #modal.worldcupMode .wcEventCenter .wcMatchesHeader{
      ...
      border-bottom:1px solid rgba(125,176,235,.18) !important;
      background:rgba(9,36,79,.44) !important;
    }
    ...
    #modal.worldcupMode .wcEventCenter .wcPickRow{
      border-radius:11px !important;
      border-color:rgba(112,164,224,.17) !important;
      background:rgba(2,15,38,.46) !important;
    }
    #modal.worldcupMode .wcEventFooter{
      padding:0 8px 8px !important;
      background:linear-gradient(180deg,rgba(4,20,49,0),rgba(4,20,49,.88) 34%) !important;
    }
    #modal.worldcupMode .wcEventFooter .wcDeadlineBox{
      min-height:48px !important;
      margin-top:7px !important;
      border-radius:14px !important;
      border-color:rgba(87,155,232,.27) !important;
      background:rgba(6,31,69,.82) !important;
      color:#fff !important;
    }
    #modal.worldcupMode .wcEventFooter .wcPlayerSaveBar{
      min-height:58px !important;
      ...
      border-radius:14px !important;
      border-color:rgba(87,155,232,.24) !important;
      background:rgba(2,15,38,.62) !important;
    }
    #modal.worldcupMode .wcEventPlayersPanel{
      padding:0 !important;
      overflow:hidden !important;
    ...
    #modal.worldcupMode .wcPlayersHeader{
      ...
      align-items:center;
      padding:12px 14px;
      border-bottom:1px solid rgba(125,176,235,.18);
      background:rgba(9,36,79,.44);
    }
    ...
    @media (min-width:981px) and (min-height:621px){
      #modal.worldcupMode .wcEventGrid{
        height:clamp(510px,calc(96vh - 220px),590px) !important;
        min-height:510px !important;
      }
    }
    @media (max-width:1260px) and (min-width:981px){
      #modal.worldcupMode .wcEventStatusBar{
        grid-template-columns:minmax(270px,1.25fr) auto minmax(185px,.85fr) minmax(150px,.7fr) 96px 96px;
        gap:6px !important;
        padding:8px !important;
      }
      #modal.worldcupMode .wcEventGrid{
        grid-template-columns:minmax(270px,.82fr) minmax(500px,1.55fr) minmax(285px,.88fr) !important;
        gap:8px !important;
      }
    }
    @media (max-width:980px), (max-height:620px){
      ...
      #modal.worldcupMode .wcEventCenter{order:1 !important;}
      #modal.worldcupMode .wcEventSummary{order:2 !important;display:grid !important;grid-template-columns:1fr 1fr !important;grid-template-rows:auto auto !important;gap:8px !important;}
      #modal.worldcupMode .wcEventSummary .wcRoundTop3Card{border-top:0 !important;border-left:1px solid rgba(125,176,235,.18) !important;}
      #modal.worldcupMode .wcEventTablePanel{grid-column:1/-1 !important;}
      #modal.worldcupMode .wcEventPlayersPanel{order:3 !important;min-height:260px !important;}
    }
    @media (max-width:620px){
      #modal.worldcupMode .modalBody{padding:6px !important;}
      #modal.worldcupMode .wcEventSummary{display:flex !important;flex-direction:column !important;}
      #modal.worldcupMode .wcEventSummary .wcRoundTop3Card{border-left:0 !important;border-top:1px solid rgba(125,176,235,.18) !important;}
      #modal.worldcupMode .wcEventCenter .wcMatchesHeader .title,
...
function wcNormalizeEventConfig(raw={}, eventId=wcCurrentEventId()){
  ...
  sourceTeams.forEach(value=>{
    const team = String(value || '').trim().slice(0, 80);
    const key = team.toLocaleLowerCase('pl');
...
function wcEventIsEnabled(){ return wcCurrentEventConfig().enabled !== false; }
function wcRequireEventActive(){
  if(wcEventIsEnabled()) return true;
  showToast(getLang()==='en' ? 'The Event is inactive.' : 'Event jest nieaktywny.');
  return false;
}
...
async function openWorldCupPlayerProfile(player){
  ...
  try{
    ...
    if(!rows.length){
      const empty = document.createElement('div');
      empty.className = 'sub';
      empty.textContent = getLang()==='en' ? 'No Event rounds to display.' : 'Brak kolejek Eventu do wyświetlenia.';
...
async function wcOpenEventSettingsModal(){
  ...
  nameLabel.textContent = getLang()==='en' ? 'Event name' : 'Nazwa Eventu';
  ...
  note.textContent = getLang()==='en'
    ? 'Changing the team list affects new rounds only. Existing matches, picks, results and ranking are not changed.'
    : 'Zmiana listy drużyn dotyczy tylko nowych kolejek. Istniejące mecze, typy, wyniki i ranking nie zostaną zmienione.';
  ...
  save.onclick = async ()=>{
    ...
    try{
      ...
      console.error('wcOpenEventSettingsModal save failed', error);
      showToast(getLang()==='en' ? 'Could not save settings' : 'Nie udało się zapisać ustawień');
      save.disabled = false;
...
function wcBuildShell(config=wcCurrentEventConfig()){
  ...
  const top=document.createElement('div'); top.className='panel wcEventStatusBar'; top.style.flexWrap='wrap'; top.style.gap='10px'; top.style.alignItems='center';
  ...
  topActions.className = 'panel wcTopActionBar';
  ...
  summary.className='panel wcEventSummary';
  summary.innerHTML = `
    <div class="wcSideSection wcTop3Card wcEventTop3Card">
    ...
    <div class="wcSideSection wcTop3Card wcRoundTop3Card">
      <div class="wcTop3Head">
    ...
    <div class="wcEventTablePanel"><div id="wcEventTableButtonHost" style="width:100%"></div></div>`;
...
function wcAskTypingDeadlineMs(defaultMs){
  return new Promise(resolve=>{
    ...
    const info = document.createElement('div');
    info.className = 'sub';
    info.textContent = getLang()==='en' ? `Choose date and time before saving the round: ${wcEventName()}.` : `Wybierz datę i godzinę przed zapisaniem kolejki: ${wcEventName()}.`;
    const input = document.createElement('input');
...
async function openWorldCupAddRoundModal(){
  ...
  if(!wcRequireEventActive()) return;
  const eventConfig = await wcGetEventConfig();
  const eventTeams = eventConfig.teams;
  const pending = [];
  ...
  const mk = (label)=>{ const wrap=document.createElement('div'); wrap.className='col'; wrap.style.flex='1'; wrap.style.minWidth='240px'; const l=document.createElement('div'); l.className='sub'; l.textContent=label; const s=document.createElement('select'); s.className='mqClubSelect'; eventTeams.forEach(t=>{ const o=document.createElement('option'); o.value=t; o.textContent=t; s.appendChild(o);}); wrap.append(l,s); return [wrap,s]; };
  ...
  modalOpen(`${getLang()==='en' ? 'Round' : 'Kolejka'} — ${eventConfig.name}`, body);
}
async function openWorldCupResultsModal(){
  ...
  modalOpen(`${getLang()==='en' ? 'Results' : 'Wyniki'} — ${wcEventName()}`, body);
  const wcModal = el('modal');
  if(wcModal) wcModal.classList.add('worldcupMode');
...
function wcShowFinalWinnersForFiveSeconds(ranking){
  return new Promise(resolve=>{
    ...
    title.innerHTML = `<span style="color:#f8fbff">${escapeHtml(wcEventName())}</span> <span style="color:#ffd35a">${isEn ? 'FINISHED' : 'ZAKOŃCZONY'}</span>`;
    const sub = document.createElement('div');
    ...
    badge.innerHTML = `<div style="font-size:34px">🏆</div><div>TYPER</div><div style="font-size:12px;color:#dbe8ff">${escapeHtml(wcEventName())}</div>`;
    ...
    modalOpen(isEn ? `${wcEventName()} finished` : `${wcEventName()} — ZAKOŃCZONY`, screen);
...
async function renderWorldCupEvent(){
  ...
  const updateWcSavePicksButton = ()=>{
    ...
    // v1012: dolny pasek ma stałe miejsce niezależnie od liczby meczów.
    // Sam przycisk pojawia się dopiero po wpisaniu wszystkich typów.
    actions.style.display = 'flex';
    btn.style.display = 'inline-flex';
    btn.style.visibility = canShow ? 'visible' : 'hidden';
    btn.style.opacity = canShow ? '1' : '0';
    btn.style.pointerEvents = canShow ? 'auto' : 'none';
  ...
  if(!matches.length){
    ...
    }else if(eventConfig.enabled === false){
      empty.className='title';
      empty.textContent=getLang()==='en'?'EVENT INACTIVE':'EVENT NIEAKTYWNY';
      empty.style.minHeight='220px';
      empty.style.display='flex';
      empty.style.alignItems='center';
      empty.style.justifyContent='center';
      empty.style.textAlign='center';
      empty.style.opacity='.82';
    }else{
      empty.className='sub';
...
async function openRoom(code, opts={}){
  ...
  unsubRoomDoc = boot.onSnapshot(ref, (d)=>{
    ...
    refreshRoomSeasonLabels();
    updateRoomProfileLeagueMini();
    updateRoomEventButtonState().catch(()=>{});
...
function renderMatches(){
  ...
  // BUILD 3087: licznik jest w stałym dolnym pasku poza przewijaną listą meczów.
  updateTypingDeadlineUI();
...
function ensureEndRoundConfirmModal(){
  ...
  // BUILD 3087: systemowe przyciski TAK/NIE zgodne z resztą gry.
